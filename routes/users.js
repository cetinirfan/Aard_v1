...
router.post('/register',(req,res,next)=>{
  const {userName,userStatus,userFinishUniversity,userUniversity,password,mail} = req.body;
...
router.post('/forgetPassword',(req,res)=>{
  ...
  const password = gnp.generate({
    length: 10,
    uppercase: true});
  const output = `<p>Merhaba yeni şifren: `+password+` </p>`;
  Users.findOne({mail},(error,Users)=>{
    if(Users){
      bcrypt.hash(password,10).then((hash)=>{
        Users.updateOne({ $set: {
            password:hash,
          ...
          .then((data)=>{
            ...
            transporter.sendMail(mailOptions, function(err,data){
              if(err){
                res.json({
                  status: true,
                  message: 'Yeni şifreniz emailinize gönderildi..'
                });
              }else{
                res.json({
                  status: false,
                  message: 'Email gönderirken bir sorun oluştu.'
                });
              }
            })
            res.json({
      ...
      res.json({
        message: 'Böyle bir email bulunamadı. Emailinizin doğru olduğundan emin olun.'
      });
    }
  });
});
